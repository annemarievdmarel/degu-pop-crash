...
theme_degu <-
  function() {
    ggpubr::theme_pubr(base_size = 12, legend = "bottom") +
...
##Top model results
summary and plots based on top models
...
```{r}
 phi.plotR1 <- ggplot(Phi.estimates.covar2, aes(x=as.numeric(year2), y=estimate, color=season2)) +  
   geom_point(position=position_dodge(0.3), size=1.5) +
  geom_errorbar(aes(ymin=lcl, ymax=ucl), 
                 width=0.25, position=position_dodge(0.3)) + #position=position_dodge(0.3),
  geom_smooth(method="lm", se=FALSE, fullrange=TRUE, size=1) +  # spans full x
  scale_x_continuous(breaks=1:12, labels=levels(Phi.estimates.covar2$year2)) +  # restore labels
  facet_grid(rows=vars(sex),  axes="all_x", labeller = labeller(sex = c("F" = "Female", "M" = "Male"))) +
  theme_classic() + theme(legend.position="bottom") +
  labs(x="Year", y=expression(phi)) +
...
```{r}
 ...
 f.plotR1 <-  ggplot(f.estimates.covar2, 
                   aes(x=as.numeric(year2), y=estimate,  color = season2)) + 
    geom_point(position=position_dodge(0.3), size=1.5) +
    geom_errorbar(aes(ymin=lcl, ymax=ucl), 
                 width=0.25, position=position_dodge(0.3))  + #position=position_dodge(0.3),
    geom_smooth(method="lm", se=FALSE, fullrange=TRUE, size=1) +  # spans full x
    scale_x_continuous(breaks=1:12, labels=levels(f.estimates.covar2$year2)) +  # restore labels
    facet_grid(rows=vars(sex),  axes="all_x", labeller = labeller(sex = c("F" = "Female", "M" = "Male"))) +
    theme_classic() + theme(legend.position="bottom") +
    labs(x="Year", y="f") +
    scale_color_manual(values=c("#40B0A6", "#E1BE6A"))
...
topmodel.beta1 <- degu.results[[kk]]$results$beta %>%
  tibble::rownames_to_column("variable") %>% 
  separate(variable, c("y", "x", "interaction")) 
  #mutate(x= gsub(x, "year2",""))
  #mutate(x = stringr::str_remove(x, " [year2]"))

write.csv(topmodel.beta1,"./results_models without covariates/beta_estimates.csv")
...
topmodel.beta <- read.csv("./results_models without covariates/beta_estimates_forplot.csv")
...
fig.betatopmodel.R1 <- topmodel.beta %>% 
  filter(x!="Intercept", y!="p") %>% 
  mutate(x=fct_rev(x)) %>% 
ggplot( aes(x=x, y=estimate)) +
  geom_hline(yintercept=0, linetype="dashed", color = "grey", size=0.8, alpha=0.8) +
  geom_point(size=1.5) + # had size = 3
  geom_linerange(aes(ymin = lcl, ymax = ucl), linewidth=1) + #linewidth=1.5
  #facet_grid(.~y) +
  facet_wrap(.~y, nrow=2,  axes="all_x")+
  theme_classic() +
  coord_flip() +
  theme( #strip.background  = element_rect(fill = "white",linewidth = 0.1),
    strip.text = element_blank(),
...
```{r}
plot.phi.f_facet <- ggarrange(phi.plotR1 + rremove("xlab"), f.plotR1 +rremove("xlab"), 
                        labels = "auto",
                        nrow=2, ncol=1, 
                        common.legend = TRUE, 
                        legend = "none")

ggarrange(plot.phi.f_facet, fig.betatopmodel.R1+rremove("xlab") ,
           nrow = 1, ncol=2, 
          align = "h", #labels = "auto", 
           widths = c(2.2,1.25),
          heights = c(2,1))

ggsave("results_models without covariates/plot_phi_f_26032026_R1_facetted.pdf", dpi = 300 , width= 180, height=150, units="mm")
```
...
ggsave("results_models without covariates/plot_phi_f_25032026_R1_legend.pdf", dpi = 300 , width= 180, height=150, units="mm")
```
...
top.p.estimates.covar2$year3 <- factor(top.p.estimates.covar2$year2 ,
                                       labels = c("'09", "'10", 
                                       "'11", "'12", 
                                       "'13", "'14", 
                                       "'15", "'16", 
...
ggplot(top.p.estimates.covar2)+
  ...
  geom_point(aes(x = time, y = estimate),position = position_dodge(0.4),size = 1.2)+
...
p.plot <- ggplot(top.p.estimates.covar2)+
  geom_line(aes(year3, estimate),alpha=0.5)+
  geom_point(aes(x = year3, y = estimate),position = position_dodge(0.0),size = 1.2, color="black")+
  geom_errorbar(aes(year3,ymax=ucl,ymin=lcl),alpha=0.7,color="cyan4", width=0.5)+
  xlab("Year") + ylab("p")+
  theme_classic() + #theme_degu() +
...
mna$season2 <- factor(mna$season, 
                      levels = c("fall", "spring"), 
                      labels = c("Breeding season", "Nonbreeding season"))
...
plot_captures <- ggplot(mna, aes(x=year, y=no_captures,  color = season2)) +
  geom_point(position=position_dodge(0.3), size=2) + # group=season
  scale_color_manual(values=c("#40B0A6", "#E1BE6A")) +
...
plot_poprateR1 <-  ggplot(pop, 
                   aes(x=year, y=N,  color = season)) + 
    geom_point(position=position_dodge(0.3), size=2) +
    geom_errorbar(aes(ymin=LCL, ymax=UCL), 
                 width=0.5, position=position_dodge(0.3))  + #position=position_dodge(0.3),
    #scale_x_continuous(breaks=1:12, labels=levels(f.estimates.covar2$year3)) +  # restore labels
...
l.plotR1 <- ggplot(lambda.time.with.covar3, 
                   aes(x=year, y=estimate, color=season2)) + 
  geom_point(position=position_dodge(0.3), size=2) +
  geom_errorbar(aes(ymin=lcl, ymax=ucl), width=0.5, position=position_dodge(0.3))+
  scale_color_manual(values =  c("#40B0A6","#E1BE6A")) +
  ...
  geom_hline(yintercept=1, linetype="dotted", color = alpha("black",0.75), size=1)+
  theme_classic()+ #theme_degu() +
...
### Fig. captures, pop size, p, and lambda

revision
```{r}
ggarrange(plot_captures+ rremove("xlab"), p.plot+ rremove("xlab"),
          plot_poprateR1, l.plotR1,  
          labels = "auto", ncol=2, nrow=2, align = "hv",
                  common.legend = TRUE, legend = "bottom")

ggsave("results_models without covariates/plot_captures_p_popsize_lambda_26032026.pdf", dpi = 300 , width= 180, height=150, units="mm")
```
...
## save R
```{r}
save.image("degu_plots_20260325.RData")
...
topmodel.beta.noyear$y <- factor(topmodel.beta.noyear$y, 
                          ...
                          labels = c("Survival", 
                                     "Recruitment"))
range(topmodel.beta.noyear$estimate)
range(topmodel.beta.noyear$lcl)
range(topmodel.beta.noyear$ucl)
                              


fig.phi.beta.noyear.r1 <- topmodel.beta.noyear %>% 
  filter(y=="Survival") %>% 
  mutate( x=factor(x, 
...
ggplot( aes(x=x1, y=estimate)) +
  ...
  theme(strip.text = element_blank(),
    strip.background = element_blank()) + # strip.background  = element_rect(fill = "white",linewidth = 0.1)) +
  labs(y="Beta estimates (95% CI)", x="")
fig.phi.beta.noyear.r1


fig.f.beta.noyear.r1 <- topmodel.beta.noyear %>% 
  filter(y=="Recruitment") %>% 
  mutate(x=factor(x, 
                  levels = c("Intercept", "GPP", "Nonbreeding"), 
                  labels = c("Intercept", "GPP", "Nonbreeding")),
         x1=fct_rev(x)) %>% 
ggplot( aes(x=x1, y=estimate)) +
  geom_hline(yintercept=0, linetype="dashed", color = "grey", size=0.8, alpha=0.8) +
   geom_point(size=1.5) +
...
ggarrange( phi_plot_ny, fig.phi.beta.noyear.r1, 
           f_plot_ny, fig.f.beta.noyear.r1,
           labels = "auto", widths = c(2.2,1.25), 
           nrow = 2, ncol = 2,
           common.legend = TRUE, align = "hv")

ggsave("results_updated models with covars/plot_topmodel_noyear_R1.pdf", dpi = 300 , width= 180, height=150, units="mm")
