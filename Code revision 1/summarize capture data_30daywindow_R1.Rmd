...
Calculate % newly captured animals each month (this would indicate new degus from surrounding area)

```{r}
levels(factor(trap$status))

newdegus <- trapdata %>% 
  mutate(status = replace(status, is.na(status), "old")) %>%
  filter(capture==1, 
         weight>170) %>% 
  separate(date, c("year", "month", "day"), sep="-") %>% 
  group_by(year, season, month, status) %>% 
  tally() %>%
   complete(#nesting(year, season, month), 
           status = c("new", "old"),
           fill = list(n = 0)) %>%
    pivot_wider(
    id_cols = c(year, season, month),
    names_from = status,
    values_from = n,
    names_prefix = "n_"  # Results in n_new, n_population
  ) %>%
  arrange(year, season, month) %>% 
  mutate(n_all = n_new+n_old, 
         per_new = n_new/n_all*100,
...
ggplot(newdegus, aes(x = year_month, y = per_new, group = 1)) +
  #geom_line(color = "blue", size = 1) +
  geom_point(color = "darkblue", size = 2) +
  facet_wrap(~ season, scales = "free_x", ncol = 1) +  # Vertical facets by season
  labs(
    title = "Proportion of new adult degus over time",
    #subtitle = "Filtered: capture==1, weight>170",
    x = "Date (Year-Month)",
    y = "Percentage New (%)"
  ) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("Revision/per_new_degusXseason.png")

newdegus_summary <- newdegus %>%
  group_by(season) %>%
  dplyr::summarise(
    avg_per_new = mean(per_new, na.rm = TRUE),       # Mean proportion (%)
    sd_per_new = sd(per_new, na.rm = TRUE),          # Standard deviation
    n_months = n(),                                  # Number of year-months
    se= sd_per_new/n_months,
    .groups = "drop"                                 # Ungroup output
  ) %>%
  arrange(season)


ggplot(newdegus_summary, aes(x = season, y = avg_per_new)) +
  geom_col() +
  labs(y = "Avg % New IDs", title = "New IDs by Season") +
  theme_classic()
```
...
head(trap_effortXwindow)
glimpse(trap_effortXwindow)
...
fall_long <- trap_effortXwindow %>% 
  filter(window == "fall") %>% 
  pivot_longer(cols = c(n_ids, n_days),
               names_to = "metric",
               values_to = "value")

# Choose how much to stretch days relative to degus:
day_scale <- max(fall_long$value[fall_long$metric == "n_ids"]) /
             max(fall_long$value[fall_long$metric == "n_days"])
# If you want days to fill more, multiply by something, e.g.:
day_scale <- day_scale /2

fall_long <- fall_long %>% 
    filter(window == "fall") %>% 
  mutate(plot_y = ifelse(metric == "n_days", value * day_scale, value))

plotfall <-   ggplot(fall_long, aes(x = year, y = plot_y, fill = metric)) +
  geom_col(stat = "identity", position = position_dodge(width = 0.9)) +
  labs(
    y = "Number of degus trapped",
    x = "Year", 
    title = "Austral Fall (June trapping window)",
    fill = "Metric"
  ) +
  theme_classic() +
  ...
  scale_y_continuous(
    limits = c(0, 150),
    sec.axis = sec_axis(
      trans = ~ . / day_scale,  # smaller day_scale = bigger days,
      name = "Number of days trapped"
    )
  ) +
  scale_fill_manual(
    values = c("#004D40", "#ff6600ff"),
    labels = c("n_ids" = "Degus trapped", "n_days" = "Days trapped")
  ) +
  geom_text(
    aes(label = value), 
...
```{r spring}
spring_long <- trap_effortXwindow %>% 
...
# Choose how much to stretch days relative to degus:
day_scale <- max(spring_long$value[spring_long$metric == "n_ids"]) /
             max(spring_long$value[spring_long$metric == "n_days"])
# If you want days to fill more, multiply by something, e.g.:
day_scale <- day_scale /2
...
plot_spring <- ggplot(spring_long, aes(x = year, y = plot_y, fill = metric)) +
  geom_col(stat = "identity", position = position_dodge(width = 0.9)) +
  labs(
    x = "Year",
    y = "Number of degus trapped",
    title = "Austral Spring (October trapping window)",
    fill = "Metric"
  ) +
  theme_classic() +
  theme(legend.position = "bottom") +
  scale_x_continuous(limits = c(2009, 2020), n.breaks = 12) +
  scale_y_continuous(
    name = "Number of degus trapped",
    limits = c(0, 510),
    sec.axis = sec_axis(
      trans = ~ . / day_scale,   # inverse of what you used on n_days
      name  = "Number of days trapped"
    )
  ) +
  scale_fill_manual(
    values = c("n_days" = "#004D40", "n_ids" = "lightblue"),
    labels = c("Days trapped", "Degus trapped")
  ) +
  geom_text(
    aes(label = value),
    position = position_dodge(width = 0.9),
...
ggsave("./data/trappingeffort_2x30daywindows_R1.png")
```
